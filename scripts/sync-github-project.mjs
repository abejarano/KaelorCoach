...
const normalize = (issue) => {
  ...
  const epic = names.find((name) => name.startsWith('epic:'))?.slice(5) ?? legacyEpic[phase];
  if (!epic) throw new Error(`#${issue.number} has no known epic or legacy phase`);
  const platform = new Set();
  if (names.some((name) => ['area:watch'].includes(name))) platform.add('platform:watch');
  if (names.some((name) => ['area:android', 'area:mobile', 'area:ui'].includes(name))) platform.add('platform:mobile');
  if (names.some((name) => ['area:backend', 'area:gcp', 'area:database'].includes(name))) platform.add('platform:api');
  const risk = new Set();
  if (names.some((name) => ['area:watch', 'area:android', 'area:health-data', 'area:ai'].includes(name))) risk.add('risk:integration');
  if (names.some((name) => ['area:health-data', 'area:health-profile', 'area:health-safety', 'area:nutrition'].includes(name))) risk.add('risk:health');
  if (names.some((name) => ['area:auth', 'area:security'].includes(name))) risk.add('risk:privacy');
